...
def New_Vertices(vor, neighbor_storage, neighbor_vertices_storage):
    """
    Replace infinite Voronoi ridge vertices (-1) with intersections on the
    bounding box by shooting a ray from the seed midpoint along the outward
    normal and choosing the smallest positive t.
    """
    ...
    next_vertex_idx = len(vor.vertices)  # next new index will be this, then +1, ...

    neighbor_vertices_storage_updated = [row.copy() for row in neighbor_vertices_storage]
    new_vertices = []
    new_vertices_index = []

    # domain center to orient outward
    ...
    for i in range(num_edges):
        ...
        # map region id -> point index in vor.points
        ...
        # unit normal to the AB segment
        ab = pb - pa
        ab /= np.linalg.norm(ab)
        n = np.array([-ab[1], ab[0]], dtype=float)

        # ensure outward (away from domain center)
        if np.dot(n, mid - center) < 0:
            n = -n

        # parametric intersections with the 4 box lines:
        # x = minx/maxx  ->  t = (x - mid.x) / n.x     (need n.x != 0)
        # y = miny/maxy  ->  t = (y - mid.y) / n.y     (need n.y != 0)
        candidates = []
        ...
        if abs(n[0]) > 1e-15:
            ...
            if t > 0:
                ...
                if miny - 1e-12 <= y <= maxy + 1e-12:
                    candidates.append((t, np.array([minx, y])))
            t = (maxx - mid[0]) / n[0]
            if t > 0:
                y = mid[1] + t*n[1]
                if miny - 1e-12 <= y <= maxy + 1e-12:
                    candidates.append((t, np.array([maxx, y])))
        ...
        if abs(n[1]) > 1e-15:
            ...
            if t > 0:
                ...
                if minx - 1e-12 <= x <= maxx + 1e-12:
                    candidates.append((t, np.array([x, miny])))
            t = (maxy - mid[1]) / n[1]
            if t > 0:
                x = mid[0] + t*n[0]
                if minx - 1e-12 <= x <= maxx + 1e-12:
                    candidates.append((t, np.array([x, maxy])))

        if not candidates:
            # No valid forward intersection → skip this ridge end
            continue

        # choose smallest positive t (closest along the ray direction)
        t_min, p_hit = min(candidates, key=lambda z: z[0])

        # add new vertex and update the -1 endpoint
        new_vertices.append([float(p_hit[0]), float(p_hit[1])])
        new_vertices_index.append(next_vertex_idx)

        # replace the -1 in ridge by the new index
        new_endpoints = ridge.copy()
        if new_endpoints[0] == -1:
            new_endpoints[0] = next_vertex_idx
        else:
            new_endpoints[1] = next_vertex_idx
        neighbor_vertices_storage_updated[i][2] = new_endpoints
