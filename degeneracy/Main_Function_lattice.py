...
def Main_Inverse_Voronoi_Lattice(vor,points):

    # minx = min(vor.vertices[:,0]) - 0.5
    # maxx = max(vor.vertices[:,0]) + 0.5
    minx = min(points[:,0]) - 0.5
    maxx = max(points[:,0]) + 0.5
    xValues = [minx, maxx]
    ...
    miny = min(points[:,1]) - 0.5
    maxy = max(points[:,1]) + 0.5
    yValues = [miny,maxy]
    ...
    from New_Vertices_lattice import New_Vertices_lattice
    new_vertices, new_vertices_index, neighbor_vertices_storage_updated = New_Vertices_lattice(vor,neighbor_storage,neighbor_vertices_storage)
   
    # --- Re-index after duplicates were merged ---
    index_map = {old_idx: new_idx for new_idx, old_idx in enumerate(sorted(set(new_vertices_index)))}
    new_vertices_index = [index_map[idx] for idx in new_vertices_index]

    # Update neighbor_vertices_storage_updated too
    for record in neighbor_vertices_storage_updated:
        record[2] = [index_map.get(v, v) for v in record[2]]   




    from New_Edges_lattice import New_Edges
    ...
    from ExplicitVoronoi_lattice import ExplicitVoronoi
