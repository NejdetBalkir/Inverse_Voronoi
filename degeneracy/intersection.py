...
def line_intersection_vertical(x, m, b):
    """Intersection of vertical line x = const with y = m*x + b."""
    if m is None:   # both vertical
        return None
    x_int = x
    y_int = m * x + b
    if np.isnan(y_int) or np.isinf(y_int):
        return None
    return [float(x_int), float(y_int)]

def line_intersection_horizontal(y, m, b):
    """Intersection of horizontal line y = const with y = m*x + b."""
    if m is None:
        return None
    if abs(m) < 1e-12:  # both horizontal
        return None
    x_int = (y - b) / m
    y_int = y
    if np.isnan(x_int) or np.isinf(x_int):
        return None
    return [float(x_int), float(y_int)]
