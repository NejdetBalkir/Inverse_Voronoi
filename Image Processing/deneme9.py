...
def extract_voronoi_graph(image_path, plot=True, merge_tol=8, vertex_tol=3):
    ...
    # --- helper: BFS skeleton path existence ---
    def skeleton_path_exists(v1, v2, max_steps=2000):
        (x1, y1), (x2, y2) = vertices[v1], vertices[v2]
    ...
    # --- pairwise ek edges (ama skeleton path ile filtreli) ---
        ...
        if len(comp_vertices) >= 2:
            for i in range(len(comp_vertices)):
                for j in range(i+1, len(comp_vertices)):
                    ...
                    if skeleton_path_exists(v1, v2):  # filtreleme
                        pairwise_edges.add(tuple(sorted((v1, v2))))

    # --- union of both edge sets ---
    edges = skeleton_edges.union(pairwise_edges)
    ...
    if plot:
        ...
        plt.title("Voronoi vertices + connectivity (skeleton + BFS-filtered pairwise)")
        plt.show()
