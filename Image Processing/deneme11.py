...
def extract_voronoi_graph(image_path, plot=True, merge_tol=8, vertex_tol=3):
    ...
    for i, vi in enumerate(uniq):
        ...
        merged.append([cx, cy])  # stored as list [x,y]
        used.add(i)

    vertices = merged  # now directly list of [x,y]
    ...
    def nearest_vertex_id(pt):
        px, py = pt
        d2 = [ (vx - px)**2 + (vy - py)**2 for (vx, vy) in vertices ]
    ...
    def add_sorted_edges(points, sortkey):
        points = sorted(points, key=lambda kv: kv[1][sortkey])
        for (vi,_),(vj,_) in zip(points, points[1:]):
            boundary_edges.add(tuple(sorted((vi,vj))))

    top = [(vid,(x,y)) for vid,(x,y) in vxys if y==miny]
    add_sorted_edges(top, 0)
    bottom = [(vid,(x,y)) for vid,(x,y) in vxys if y==maxy]
    add_sorted_edges(bottom, 0)
    left = [(vid,(x,y)) for vid,(x,y) in vxys if x==minx]
    add_sorted_edges(left, 1)
    right = [(vid,(x,y)) for vid,(x,y) in vxys if x==maxx]
    add_sorted_edges(right, 1)

    # --- union ---
    edges = skeleton_edges.union(boundary_edges)
    ...
    if plot:
        ...
        plt.title("Voronoi vertices + connectivity (skeleton + bbox edges)")
        plt.show()

    # edges'i list of [i,j] formatında döndür
    edge_list = [[a,b] for (a,b) in edges]

    return vertices, connectivity, edge_list


if __name__ == "__main__":
    image_path = "C:\\Users\\user\\OneDrive\\PHD\\Repositories\\Inverse_Voronoi\\Image Processing\\voronoi_image.png"
    vertices, connectivity, edge_list = extract_voronoi_graph(image_path, merge_tol=5, vertex_tol=8)
    print("Extracted vertices:", vertices)       # list of [x,y]
    print("Connectivity:", dict(connectivity))  # adjacency
    print("Edges:", edge_list)                  # list of [i,j]
